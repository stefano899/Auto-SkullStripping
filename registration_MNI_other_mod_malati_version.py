...
def find_modality_file(folder: Path, modality: str) -> Path | None:
    ...
    for f in files:
        ...
        # escludi maschere (non consensus)
        if "mask" in name:
            continue
...
def find_consensus_dirs_for_subject(subj_dir: Path) -> list[Path]:
    """
    Trova cartelle chiamate esattamente 'consensus' (case-insensitive)
    all'interno della cartella del soggetto. Ricerca ricorsiva.
    """
    dirs = []
    for p in subj_dir.rglob("*"):
        if p.is_dir() and p.name.lower() == "consensus":
            # evita cartelle di servizio
            if "_transforms" in str(p).lower():
                continue
            dirs.append(p)
    # ordina per profondità relativa crescente
    dirs.sort(key=lambda d: len(d.relative_to(subj_dir).parts))
    return dirs
...
def main():
    ...
    for i, (subject, rel_skull) in enumerate(iterator, start=1):
        ...
        # nome di output T1: stesso nome + _to_mni
        ...
        # === NEW: CONSENSUS (cartella 'consensus' dentro al soggetto) ===
        try:
            cons_dirs = find_consensus_dirs_for_subject(subj_src_root)
            if not cons_dirs:
                print("  ℹ️ Nessuna cartella 'consensus' trovata per questo soggetto, salto.")
            else:
                for cons_dir in cons_dirs:
                    rel_cons_dir = cons_dir.relative_to(subj_src_root)             # es: "anat/consensus" o "ses-01/anat/consensus"
                    dst_cons_dir = dst_root / subject / rel_cons_dir               # mirror della struttura
                    dst_cons_dir.mkdir(parents=True, exist_ok=True)

                    # prendi tutti i file .nii/.nii.gz nella cartella consensus
                    cons_files = list(cons_dir.glob("*.nii")) + list(cons_dir.glob("*.nii.gz"))
                    if not cons_files:
                        print(f"  ℹ️ Nessun file NIfTI in {cons_dir}, continuo.")
                        continue

                    for cons_file in cons_files:
                        cons_dst = dst_cons_dir / cons_file.name  # stesso nome, nessun suffisso
                        if cons_dst.exists():
                            print(f"  ⏩ Consensus già registrato: {cons_dst}")
                            continue

                        print(f"  ▶ Registro CONSENSUS → MNI (warp+affine, NN): {cons_file.name}")
                        cons_native = ants.image_read(str(cons_file))
                        cons_in_mni = ants.apply_transforms(
                            fixed=ants.image_read(MNI_T1),   # stessa grid del template MNI
                            moving=cons_native,
                            transformlist=t1_to_mni_chain,   # catena completa T1→MNI (warp + affine)
                            interpolator="nearestNeighbor"   # preserva lesioni/etichette
                        )
                        ants.image_write(cons_in_mni, str(cons_dst))
                        print(f"    ✅ Salvato CONSENSUS: {cons_dst}")
        except Exception as e:
            print(f"  ⚠️ Errore durante la gestione CONSENSUS: {e}")

        # 3) FLAIR e T2 (come prima)
        for mod in ("FLAIR", "T2"):
    ...
    print("\n[TUTTO FATTO] Registrazione su MNI completata: T1/FLAIR/T2 + CONSENSUS (cartella) con struttura mirror.")
    return 0
